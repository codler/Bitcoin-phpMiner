...
class Util {
	...
	// needed for 64bits
	function uint32_left_shift($a, $b) {
		// check if negative
	...
	function uint32_array_to_hex($arr) {
		# PHP 5.3+
	...
	function hex_to_uint32_array($h) {
		return array_map(hexdec, str_split($h, 8));
	}
	
	function reverse_bytes_in_word($w) {
	...
	function reverse_bytes_in_words($words) {
		$reversed = array();
